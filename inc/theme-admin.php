...
$tinker_colors = array(
		...
		'header-color' => array( 
			...
			'default' => '222222',
			'css' => array( 
				'#header, .breadcrumb, .pagination, .wrap-footer, .index-header, .index-heading, .index-header a, .comments .comments-title' => 'color',
...
foreach ( $tinker_google_fonts as $font_uri )
	if ( $font_uri_parts = explode( ':', $font_uri ) )
		$tinker_font_choices[ $font_uri ] = str_replace( '+', ' ', $font_uri_parts[0] );
